...
const walk = (maze: string[], wall: string, current: Point, end: Point, seen: boolean[][], path: Point[]): boolean => {
  if(current.x < 0 || current.x >= maze[0].length || current.y < 0 || current.y >= maze.length) return false
  ...
  for (let i = 0; i < dir.length; ++i) {
    const [x, y] = dir[i]
    if(walk(maze, wall, {x: current.x + x, y: current.y + y}, end, seen , path)) return true
  }
